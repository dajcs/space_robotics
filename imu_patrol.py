...
x_time = RECTANGLE_X / LINEAR_SPEED   # [s] 5
y_time = RECTANGLE_Y / LINEAR_SPEED   # [s] 2.5
...
class LeoRover(Node):
    ...
    def imu_msg_callback(self, msg):
        ...
        if self.turn == False:                  # go ahead
            ...
            cmd = f'Linear drive, speed = {self.linear_speed} m/s,  rectangle X latitude: {self.latitude_X})'
            self.drive_time += dt
            ...
            if self.theta > pi / 2:
                self.theta = 0
                self.turn = False
                self.latitude_X = not self.latitude_X      # alternating rect_x and rect_y
                self.latitude_time = x_time if self.latitude_X else y_time
